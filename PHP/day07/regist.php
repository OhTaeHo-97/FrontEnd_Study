...
<body>
    ...
    <form action = "regist_ok.php" name="regform" id="regform" method="post" onsubmit = "return sendit()">
        <!-- onsubmit = "return sendit()" -> 이 form이 제출되면서 sendit 함수를 호출할 것임, sendit이 true면 제출, false면 제출하지 않음 -->
        ...
        <p>취미 : <label>드라이브<input type="checkbox" name="hobby[]" value="드라이브"></label>
            ...
            <label>코딩<input type="checkbox" name="hobby[]" value="코딩"></label></p>
        ...
        <!-- onkeyup="moveFocus()" -> 키보드를 치다가 위로 키보드가 올라왔을 때(누를 때는 keydown, 그 손가락을 뗄 때가 keyup(키보드가 올라올 때)) moveFocus라는 함수를 호출하겠다는 뜻 -->
        <p>
